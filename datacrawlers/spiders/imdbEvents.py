...
class ImdbeventsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        requests = [SplashRequest(url=event, callback=self.parse_event, args={'wait': 1.5}) for event
                    in events]
        return requests
    ...
    def parse_event(self, response):
        ...
        requests = [SplashRequest(url="https://www.imdb.com/" + year, callback=self.parse_page, args={'wait': 1.5}) for
                    year
                    in events_year]
        return requests
    ...
    def parse_page(self, response):
        ...
        with open('dataImdbMovieEvents.json', 'a') as outfileM:
            with open('dataImdbActorEvents.json', 'a') as outfileA:
                for product in response.xpath('//div[@class="event-widgets__award-category"]'):
